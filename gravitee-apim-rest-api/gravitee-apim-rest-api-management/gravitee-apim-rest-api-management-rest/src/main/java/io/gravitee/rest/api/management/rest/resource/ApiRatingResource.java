...
public class ApiRatingResource extends AbstractResource {
    ...
    public RatingEntity getApiRatingByApiAndUser() {
        ...
        final ApiEntity apiEntity = apiService.findById(GraviteeContext.getExecutionContext(), api);
        if (PUBLIC.equals(apiEntity.getVisibility()) || hasPermission(RolePermission.API_RATING, api, RolePermissionAction.READ)) {
            return filterPermission(api, ratingService.findByApiForConnectedUser(GraviteeContext.getExecutionContext(), api));
        } else {
            throw new UnauthorizedAccessException();
        }
    }
    ...
    public RatingSummaryEntity getApiRatingSummaryByApi() {
        final ApiEntity apiEntity = apiService.findById(GraviteeContext.getExecutionContext(), api);
        if (PUBLIC.equals(apiEntity.getVisibility()) || hasPermission(RolePermission.API_RATING, api, RolePermissionAction.READ)) {
            return ratingService.findSummaryByApi(GraviteeContext.getExecutionContext(), api);
        } else {
            throw new UnauthorizedAccessException();
        }
    }
